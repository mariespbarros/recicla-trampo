package br.com.fiap.reciclatrampo.data.auth

import android.content.Context
import androidx.room.Database
import androidx.room.Room
import androidx.room.RoomDatabase
import androidx.room.TypeConverters
import br.com.fiap.reciclatrampo.data.Converters
import br.com.fiap.reciclatrampo.data.dao.ColetaDao
import br.com.fiap.reciclatrampo.model.Coleta

@Database(
    entities = [UserEntity::class, Coleta::class],
    version = 2
)
@TypeConverters(Converters::class)
abstract class AppDatabase : RoomDatabase() {

    abstract fun userDao(): UserDao

    abstract fun coletaDao(): ColetaDao

    companion object {
        @Volatile
        private var INSTANCE: br.com.fiap.reciclatrampo.data.auth.AppDatabase? = null

        fun getDatabase(context: Context): br.com.fiap.reciclatrampo.data.auth.AppDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(
                    context.applicationContext,
                    AppDatabase::class.java,
                    "recicla_database"
                ).allowMainThreadQueries().build()
                INSTANCE = instance
                instance
            }
        }
    }

}